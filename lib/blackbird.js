...
( function() {
	...
	function generateMarkup() { //build markup
		...
		for ( type in messageTypes ) {
			spans.push( [ '<span class="bb_', type, '" type="', type, '"></span>'].join( '' ) );
		}
		...
		newNode.innerHTML = [
			'<div class="bb_header">',
				'<div class="bb_left">',
					'<div id="', IDs.filters, '" class="bb_filters" title="click to filter by message type">', spans.join( '' ), '</div>',
				'</div>',
				'<div class="bb_right">',
					'<div id="', IDs.controls, '" class="bb_controls">',
						'<span id="', IDs.size ,'" title="contract" op="resize"></span>',
						'<span class="bb_clear" title="clear" op="clear"></span>',
						'<span class="bb_close" title="close" op="close"></span>',
					'</div>',
				'</div>',
			'</div>',
			'<div class="bb_main">',
				'<div class="bb_left"></div><div class="bb_mainBody">',
					'<ol>', cache.join( '' ), '</ol>',
				'</div><div class="bb_right"></div>',
			'</div>',
			'<div class="bb_footer">',
				'<div class="bb_left"><label for="', IDs.checkbox, '"><input type="checkbox" id="', IDs.checkbox, '" />Visible on page load</label></div>',
				'<div class="bb_right"></div>',
			'</div>'
		].join( '' );
		return newNode;
	}
	...
	function addMessage( type, content ) { //adds a message to the output list
		...
		if ( outputList ) {
			...
			newMsg.className = 'bb_' + type;
			newMsg.innerHTML = [ '<span class="icon"></span>', content ].join( '' );
			outputList.appendChild( newMsg );
			scrollToBottom();
		} else {
			cache.push( [ '<li class="bb_', type, '"><span class="icon"></span>', content, '</li>' ].join( '' ) );
		}
	}
		...
		if ( span && span.tagName == 'SPAN' ) {
			...
			if ( evt.altKey ) {
				...
				for ( var i = 0; filters[ i ]; i++ ) {
					...
					filters[ i ].className = 'bb_' + ( oneActiveFilter || ( spanType == type ) ) ? spanType : spanType + 'Disabled';
					messageTypes[ spanType ] = oneActiveFilter || ( spanType == type );
				}
			}
			else {
				messageTypes[ type ] = ! messageTypes[ type ];
				span.className = 'bb_' + ( messageTypes[ type ] ) ? type : type + 'Disabled';
			}
		...
		profile: 
			function( label ) {
				...
				if ( label == undefined || label == '' ) {
					...
					// ** don't generate these lines ** addMessage( 'profile', label );
				}
				return currentTime;
			}
	}
